...
export const metadata = { title: "Template Test — MarginLab" };

const analyticsService = new AnalyticsService();

export default async function TemplateTestDetailPage({
  params,
  searchParams,
}: {
  ...
  const experiment = await prisma.experiment.findFirst({
    where: { id, shopId: shop.id, type: "TEMPLATE_TEST" },
    select: {
  ...
  return (
    <ExperimentDetailShell
      ...
      tab={tab}
      breadcrumb={{ href: "/template-tests", label: "Template Tests" }}
    />
  );
}
